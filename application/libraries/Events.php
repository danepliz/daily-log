...
class Events {
	...
	public static function trigger($event, $data = '', $return_type = 'string')
	{
		...
		return self::_format_return($result, $return_type);
	}
	 ...
	 */
	protected static function _format_return($calls, $return_type)
	{
	...
	public static function has_listeners($event)
	{
		...
		{
			log_message('debug', 'Events::has_listeners() - "'.$event.'" has listeners.');
			return TRUE;
		}
		log_message('debug', 'Events::has_listeners() - "'.$event.'" does not have any listeners.');
		return FALSE;
	}
	
	public static function get_listeners($event){
		log_message('debug', 'Events::get_listeners() - Checking if event "'.$event.'" has listeners.');
