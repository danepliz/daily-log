...
class OnlineMenu{
	...
	public static function register(\OnlineMenuItem &$menuitem, $beforeAfterFlag = NULL, $targetId = NULL){
...
// 			$parentMenu = self::$registry[$menuitem->getParent()->getId()];
	...
	private static function wrap($source){
		...
		usort($source,array("OnlineMenu","_compare"));
...
// 			}
			...
			$output .= "<a href='{$route}'>{$v->getName()}</a>";
